...
- 实现了字符串操作扩展功能，提供更强大的文本处理能力：
  - `string.split` - 将字符串按指定分隔符分割成数组，支持空分隔符
  - `string.replace` - 替换字符串中的指定内容，默认替换所有匹配
  - `string.trim` - 去除字符串两端的空白字符
  - `string.format` - 使用模板和参数对象格式化字符串，支持`{name}`形式的占位符
- 字符串操作扩展功能的特点：
  - 灵活的参数格式，支持直接值和变量引用
  - 强大的文本处理能力，覆盖常见的字符串操作需求
  - 统一的错误处理机制，提供友好的错误提示
  - 完整的嵌套变量支持，可在格式化字符串中引用复杂数据结构
  - 可组合使用，实现复杂的文本解析和转换功能

### 架构改进

- 扩展了字符串处理模块：
  - 在`src/statements/string.rs`中添加了四个新的字符串操作处理器
  - 每个处理器遵循统一的设计模式，专注于特定的字符串操作功能
  - 实现了完整的参数验证和错误处理机制
- 增强了变量处理能力：
  - 优化了`string.format`处理器中的变量解析和求值机制
  - 确保格式化字符串中的变量引用能够被正确解析和替换
  - 提高了字符串操作与嵌套变量路径的兼容性
- 完善了测试系统：
  - 为每个新增的字符串操作函数添加了专门的单元测试
  - 测试覆盖了基本功能和边缘情况处理
  - 确保所有操作的正确性和一致性
...
- 添加了字符串操作的示例程序：
  - 创建了`examples/string_operations_demo.njis`，全面展示所有字符串操作功能
  - 演示了分割、替换、修剪和格式化等基本操作
  - 提供了组合使用示例，如CSV数据解析和格式化输出
- 更新了TODO列表，标记已完成的字符串操作相关功能
- 为所有字符串操作处理器提供了详细的代码注释和使用说明

## [0.2.6] - 2025-06-02

### 新增功能

- 实现了异常处理机制，提供完整的异常捕获与抛出能力：
  - `throw` - 抛出异常，支持任意类型的异常值（字符串、对象等）
  - `try/catch` - 异常捕获机制，捕获代码执行过程中产生的异常
- 异常处理功能的特点：
  - 支持结构化异常信息，可以传递复杂的异常对象（包含错误代码、消息、附加数据等）
  - 异常可以在嵌套的函数调用栈中传播，被适当的地方捕获处理
  - 可以捕获用户显式抛出的异常以及系统内部错误（如未定义的变量、语法错误等）
  - 提供异常变量机制，在catch块中通过变量访问异常信息
  - 支持嵌套的try/catch结构，可以实现复杂的异常处理流程
  - 允许在catch块中重新抛出异常，方便异常转换和传递

### 架构改进

- 扩展了错误处理系统：
  - 在 `NjilError` 枚举中添加了 `ThrowException` 变体，专门用于表示用户抛出的异常
  - 完善了异常信息的格式化和显示机制
- 创建了异常处理模块，遵循模块化设计原则：
  - 添加了 `src/statements/throw.rs`，实现了异常抛出功能
  - 添加了 `src/statements/try_catch.rs`，实现了异常捕获功能
  - 在 `src/statements/mod.rs` 中注册了新的异常处理器
- 完善了错误提示信息系统：
  - 在 `errortip.rs` 中添加了 `exception` 模块，包含异常处理相关的错误提示
  - 提供了详细的错误提示信息，帮助用户理解和修复异常处理中的问题
- 添加了单元测试，确保异常处理功能的正确性：
  - 测试异常抛出的不同场景（字符串异常、对象异常等）
  - 测试try/catch结构的异常捕获能力
  - 测试嵌套异常处理和异常传播机制

### 文档和示例

- 更新了TODO列表，标记已完成的异常处理功能
- 为异常处理功能提供了详细的代码注释和错误处理说明
- 添加了示例程序 `examples/exception_demo.njis`，展示异常处理的不同用法：
  - 基本的异常抛出和捕获
  - 结构化异常对象的使用
  - 系统错误（如未定义变量）的捕获
  - 嵌套异常处理和异常重新抛出
